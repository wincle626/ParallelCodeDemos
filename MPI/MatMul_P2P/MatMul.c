...
int main(int argc, char *argv[])
{
    ...
    if (myrank==0) {
        ...
        for(i=1;i<np;i++){
            int dest = i;
            MPI_Send(&vA[i*DIM1*DIM2/np], DIM1*DIM2/np, MPI_INT, 
        ...
        for(i=1;i<np;i++){
            ...
            MPI_Recv(&vC[i*DIM1*DIM3/np], DIM1*DIM3/np, MPI_INT,
                    sour, tag, MPI_COMM_WORLD, &status);
            // memcpy(&vC[i*DIM1*DIM3/np], vCpart, sizeof(int)*DIM1*DIM3/np);
        }

        // reverse from vector to matrix
        vector2matrix(DIM1, DIM3, vC, C);
        ...
        // allocate memory 
        vA = allocatevector(DIM1*DIM2/np);
        vB = allocatevector(DIM2*DIM3);
        ...
        int sour = 0;
        int dest = 0;
        MPI_Recv(vA, DIM1*DIM2/np, MPI_INT, 
                sour, tag, MPI_COMM_WORLD, &status);
        MPI_Recv(vB, DIM2*DIM3, MPI_INT, 
                sour, tag, MPI_COMM_WORLD, &status);
