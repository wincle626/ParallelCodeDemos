...
int main(int argc, char *argv[])
{
    ...
    if(argc<2){
        printf("Please input a common divider\n");
        MPI_Finalize();
        exit(-1);
    }else{
        cmd = atoi(argv[1]);
    ...
    if (myrank==0) {
        ...
        // worker 1
        int dest = 1;
        MPI_Send(&vA[0*DIM1*DIM2/cmd], DIM1*DIM2/cmd, MPI_INT, 
                dest, tag, MPI_COMM_WORLD);
        MPI_Send(&vB[DIM2*DIM3/cmd], DIM2*DIM3/cmd, MPI_INT, 
                dest, tag, MPI_COMM_WORLD);
        // worker 2
        dest = 2;
        MPI_Send(&vA[1*DIM1*DIM2/cmd], DIM1*DIM2/cmd, MPI_INT, 
                dest, tag, MPI_COMM_WORLD);
        MPI_Send(&vB[0*DIM2*DIM3/cmd], DIM2*DIM3/cmd, MPI_INT, 
                dest, tag, MPI_COMM_WORLD);
        // worker 3
        dest = 3;
        MPI_Send(&vA[1*DIM1*DIM2/cmd], DIM1*DIM2/cmd, MPI_INT, 
                dest, tag, MPI_COMM_WORLD);
        MPI_Send(&vB[1*DIM2*DIM3/cmd], DIM2*DIM3/cmd, MPI_INT, 
                dest, tag, MPI_COMM_WORLD);

        // do the matrix multiplication on host
        for(i=0;i<DIM1/cmd;i++){
            for(j=0;j<DIM3/cmd;j++){
                C[i][j] = 0;
                for(k=0;k<DIM2;k++){
        ...
        int *vCpart = (int *) malloc(sizeof(int)*DIM1*DIM3/cmd/cmd);
        // worker 1
        int sour = 1;
        MPI_Recv(vCpart, DIM1*DIM3/cmd/cmd, MPI_INT,
                sour, tag, MPI_COMM_WORLD, &status);
        for(k=0;k<DIM1/cmd;k++){
            for(j=0;j<DIM3/cmd;j++){
                C[0*DIM1/cmd+k][DIM3/cmd+j] = vCpart[k*DIM3/cmd+j];
            }
        }
        // worker 2
        sour = 2;
        ...
        for(k=0;k<DIM1/cmd;k++){
            for(j=0;j<DIM3/cmd;j++){
                C[1*DIM1/cmd+k][0*DIM3/cmd+j] = vCpart[k*DIM3/cmd+j];
            }
        }
        // worker 3
        sour = 3;
        ...
        for(k=0;k<DIM1/cmd;k++){
            for(j=0;j<DIM3/cmd;j++){
                C[1*DIM1/cmd+k][1*DIM3/cmd+j] = vCpart[k*DIM3/cmd+j];
            }
        }
        free(vCpart);
        ...
        // allocate memory 
        vA = allocatevector(DIM1*DIM2/cmd);
        vB = allocatevector(DIM2*DIM3/cmd);
        vC = allocatevector(DIM1*DIM3/cmd/cmd);
        
        // receive data from host
        int sour = 0;
        int dest = 0;
        MPI_Recv(vA, DIM1*DIM2/cmd, MPI_INT, 
                sour, tag, MPI_COMM_WORLD, &status);
        MPI_Recv(vB, DIM2*DIM3/cmd, MPI_INT, 
                sour, tag, MPI_COMM_WORLD, &status);
        ...
        for(i=0;i<DIM1/cmd;i++){
            for(j=0;j<DIM3/cmd;j++){
                vC[i*DIM3/cmd+j] = 0;
                for(k=0;k<DIM2;k++){
                    vC[i*DIM3/cmd+j] += vA[i*DIM2+k] * vB[j*DIM2+k];
                }
            }
        }
        
        // send to host
        MPI_Send(vC, DIM1*DIM3/cmd/cmd, MPI_INT, 
                dest, tag, MPI_COMM_WORLD);
