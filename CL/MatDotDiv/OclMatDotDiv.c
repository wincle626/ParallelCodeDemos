...
int main() {
    ...
    size_t datasize = sizeof(float)*ROW*COL;
    ...
    // Load the kernel source code floato the array source_str
    FILE *fp;
    char *source_str;
    size_t source_size;
    fp = fopen("mdd.cl", "r");
    ...
    kernel = clCreateKernel(program, "matrix_dot_div", &status);
    if(status==CL_SUCCESS){
...
#if DEBUG
            printf("C[%d][%d]=%f - %f\n",
                    i,j,C[i*COL+j],(float)(i*COL+j+1)/(i*COL+j+1));
#endif
            if((int)C[i*COL+j] != (i*COL+j+1)/(i*COL+j+1)) {
                result = false;
                break;
            }
        }
